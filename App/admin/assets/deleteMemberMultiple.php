...
if ($_SERVER['REQUEST_METHOD'] == 'POST')
{
	...
	try
	{
		...
		if(NoCSRF::check( 'csrf_token_delete_member_multiple', $_POST, true, 60 * 5, false ));
	}
	catch ( Exception $e )
	{
	...
	if (empty($_POST['memberDeleteMultiple'])) {
		$errSelection = "Select member(s) to delete.";
	}
}

//Check all fields
if (empty($_POST['memberDeleteMultiple']) || !empty($errCSRF) || !empty($errSelection) || $_SERVER['REQUEST_METHOD'] == 'GET')
{
	if ($_SERVER['REQUEST_METHOD'] == 'POST')
   	{
   		if (!empty($errCSRF)) {
...
{	
	...
	$memberIds = $_POST['memberDeleteMultiple'];
	
	$result = $db->deleteMemberMultiple($memberIds);
		
	if($result)
	{
		$db->commit();

		$_SESSION['notify_success_delete_member'] = "Member(s) successfully deleted.";
	...
	else
	{
		...
		$_SESSION['notify_error_delete_member'] = "Error while deleting member(s). Contact admin.";
